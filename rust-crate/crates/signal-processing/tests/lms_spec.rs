...
use signal_processing::adaptive_filter::lms_filter;
...
#[test]
fn lms_converges_on_fir_system_identification() {
    // 既知のFIRシステム h を LMS で同定できるか（平均二乗誤差が減少するか）
    let fs = 1000.0;
    let h = Polynomial::new(vec![0.25, -0.15, 0.1, 0.05]);
    ...
    // LMS は μ を小さめに（NLMSよりスケーリングに敏感）
    let (_yo, eo) = lms_filter(&xsig, &dsig, taps, 0.01);
    let e = eo.data();
    let mid = n / 2;
