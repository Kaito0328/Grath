...
pub fn is_primitive_gf2(poly: &[GFp<2>]) -> bool {
    // degree m
    let _m = match poly.len().checked_sub(1) {
        Some(d) if d >= 1 => d,
        _ => return false,
    };
    if poly.last() != Some(&GFp::<2>(1)) || poly.first() != Some(&GFp::<2>(1)) {
        return false;
    }
...
pub fn is_irreducible_gf2(poly: &[GFp<2>]) -> bool {
    let m = match poly.len().checked_sub(1) {
        Some(d) if d >= 1 => d,
        _ => return false,
    };
    ...
    for d in 1..=m / 2 {
        ...
        for mask in 0..count {
            ...
            for i in 1..d {
                q[i] = GFp::<2>(((mask >> i) as u16) & 1);
            }
            q[d] = GFp::<2>(1);
            // compute gcd(poly, q)
            if poly_gcd_gf2(poly, &q).len() > 1 {
                return false;
            }
        }
    }
...
pub fn poly_rem_gf2(a: &[GFp<2>], b: &[GFp<2>]) -> Vec<GFp<2>> {
    ...
    while r.len() >= d.len() {
        ...
        // r = r + x^{shift} * d
        for i in 0..=db {
            r[shift + i] = GFp::<2>(r[shift + i].0 ^ d[i].0);
        }
        r = trim(r);
    }
    r
...
fn trim(mut v: Vec<GFp<2>>) -> Vec<GFp<2>> {
    while v.len() > 1 && *v.last().unwrap() == GFp::<2>(0) {
        v.pop();
    }
    v
}
...
pub fn primitive_table_gf2(max_m: usize) -> Vec<Vec<GFp<2>>> {
    let mut out = Vec::new();
    for m in 2..=max_m {
        if let Some(p) = find_primitive_gf2(m) {
            out.push(p);
        } else {
            out.push(vec![]);
        }
...
pub fn find_primitive_gf2(m: usize) -> Option<Vec<GFp<2>>> {
    ...
    for mask in 0..total {
        ...
        p[0] = GFp::<2>(1);
        for i in 1..m {
            p[i] = GFp::<2>(((mask >> (i - 1)) as u16) & 1);
        }
        p[m] = GFp::<2>(1);
        if is_primitive_gf2(&p) {
            return Some(p);
        }
    }
    None
