...
use polynomial::core::Polynomial as Poly;
use std::sync::Arc;
...
impl FiniteField2m {
    ...
    pub fn new(px: &[GFp<2>]) -> Self {
        ...
        assert!(
            px[0].0 == 1,
            "constant coeff should be 1 for primitive polynomials (usual)"
        );
        // pack bits
        let mut bits: u32 = 0;
    ...
    #[inline]
    pub fn is_zero(&self, a: u16) -> bool {
        a == 0
    }
    ...
    pub fn px_gf2_coeffs(&self) -> Vec<GFp<2>> {
        ...
        for i in 0..=self.m {
            let bit = ((self.px_bits >> i) & 1) as u16;
            v.push(GFp::<2>(bit));
        }
        v
    }
    ...
    pub fn px_arc(&self) -> Arc<Vec<GFp<2>>> {
        Arc::new(self.px_gf2_coeffs())
    }
