...
impl BlockHuffmanTree {
    pub fn new(q: usize, symbols_pr: SymbolsPr) -> Self {
        Self {
            q: q.max(2),
            symbols_pr,
        }
    ...
    pub fn huffman_code(&self) -> SymbolsToCodeWord {
        ...
        if nodes.len() == 1 {
            ...
            if let Some(sym) = nodes[0].sym.clone() {
                map.insert(sym, vec![0]);
            }
            return map;
        }
        fn dfs(
        ...
        for (sym, code) in &codes {
            ...
            for &d in code {
                ...
                if node.children.is_empty() {
                    node.children = mk_children(self.q);
                }
                if node.children[d].is_none() {
                    node.children[d] = Some(Box::new(Trie {
                        children: mk_children(self.q),
                        sym: None,
                    }));
                }
