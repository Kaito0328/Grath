    ...
    fn eigen_decomposition(&self) -> crate::Result<Eigen> {
            ...
            loop {
                if total_iterations >= max_total_iterations {
                    ...
                    return Err(crate::LinalgError::InvalidArgument {
                        text: "Maximum total iterations reached".into(),
                    });
                }
                total_iterations += 1;
                ...
                if let Ok(qr) = shifted_h.submatrix(0, end, 0, end).qr_decomposition() {
                    let q = qr.q;
                    ...
                    return Err(crate::LinalgError::InvalidArgument {
                        text: "QR decomposition failed".into(),
                    });
                }
            }
        }
        ...
        for &lambda in &eigenvalues_unsorted {
            ...
            if let Some(vec) = b.nullspace_vector(tol_ns) {
                ...
                return Err(crate::LinalgError::InvalidArgument {
                    text: "Failed to compute nullspace vector".into(),
                });
            }
        }
...
impl Matrix<f64> {
    ...
    fn qr_iteration_to_schur(h: &mut Matrix<f64>, q: &mut Matrix<f64>, tol: f64) -> bool {
        ...
        while end > 0 {
            ...
            for _ in 0..max_iter {
                let Some(qr) = h.submatrix(0, end, 0, end).qr_decomposition().ok() else {
                    return false;
                };
                let q_step = qr.q;
                let r_step = qr.r;
