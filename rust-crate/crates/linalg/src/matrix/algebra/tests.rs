...
#[test]
fn test_symbolic_inverse_simplification() {
    use crate::matrix::Matrix;
    use algebraic::prelude::*;

    // A = [[a, 0], [0, 1]]
    let a_sym = SymbolicExpr::Symbol("a".to_string());
    ...
    let mat = Matrix::new(
        2,
        2,
        vec![a_sym.clone(), zero.clone(), zero.clone(), one.clone()],
    )
    .unwrap();

    let inv = mat.inverse_exact().unwrap().expect("Invertible");
    println!("Inverse of [[a, 0], [0, 1]] = \n{}", inv);

    // Expected simple inverse: [[1/a, 0], [0, 1]]
    let expected_00 = SymbolicExpr::pow(a_sym.clone(), SymbolicExpr::int(-1)).simplify();
    let actual_00 = inv[(0, 0)].clone();

    println!("Actual (0,0): {}", actual_00);
    println!("Expected (0,0): {}", expected_00);

    // If it's not simplified, this might fail or show the complex expression
