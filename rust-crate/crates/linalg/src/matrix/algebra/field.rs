...
impl<T: Field> Matrix<T> {
    ...
    /// Find a basis for the kernel (null space) of the matrix.
    /// The matrix should be in RREF.
    pub fn find_kernel_basis_from_rref(&self) -> Result<Vec<Vector<T>>>
    where
        T: PartialEq,
    {
        let rows = self.rows;
        ...
        for c in 0..cols {
            if !pivot_cols.contains(&c) {
                ...
                let mut vec_data = vec![T::zero(); cols];
                vec_data[c] = T::one();
                for (pr, pc) in &pivots {
                    vec_data[*pc] = T::zero() - self[(*pr, c)].clone();
                }
                basis.push(Vector::new(vec_data));
            }
        }
