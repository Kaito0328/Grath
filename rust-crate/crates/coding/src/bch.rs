use crate::codec_common::{bch_generator_poly, systematic_cyclic_encode_full};
...
impl BCHCode {
    ...
    pub fn decode_lut(
        &self,
        r: &crate::types::Codeword<GFp<2>>,
    ) -> crate::error::Result<crate::types::Codeword<GFp<2>>> {
        let k = self.k;
        let n = self.n;
        // 巡回符号の標準的な生成行列: 各行は x^i * g(x) を x^n-1 で縮約した長さ n のベクトル
        let mut gmat = linalg::Matrix::new(k, n, vec![GFp::<2>(0); k * n]).map_err(|e| {
            crate::error::CodingError::InvalidParameters {
    ...
    pub fn decode_bm(
        ...
        let n = self.n;
        if r.dim() != n {
            return Err(crate::error::CodingError::InvalidParameters {
                text: format!("received length {} must equal n {}", r.dim(), n),
            });
        }
        // 2t syndromes over GF(2^m) as GFExt elements
        let synd = bch_compute_syndromes_binary(&self.field, &r.as_ref().data, n, self.t, b);
        if synd.iter().all(|x| x.is_zero()) {
            return Ok(r.clone());
        }
        ...
        if positions.len() > self.t {
            return Err(crate::error::CodingError::DecodeFailure {
