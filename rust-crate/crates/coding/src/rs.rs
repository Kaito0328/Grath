use crate::codec_common::{
    rs_chien_positions_over_indices, rs_compute_syndromes_over_indices, rs_forney_values,
...
impl ReedSolomon {
    pub fn encode(&self, f: &Message<GFExt<GFp<2>>>) -> CodingResult<Codeword<GFExt<GFp<2>>>> {
        if f.dim() != self.k {
            return Err(CodingError::InvalidParameters {
                text: format!("message length {} must equal k {}", f.dim(), self.k),
            });
        }
    ...
    pub fn decode(
        &self,
        r: &Codeword<GFExt<GFp<2>>>,
    ) -> CodingResult<RSDecodeResult<GFExt<GFp<2>>>> {
        if r.dim() != self.n {
            return Err(CodingError::InvalidParameters {
                text: format!("received length {} must equal n {}", r.dim(), self.n),
            });
        }
        let rvec = r.as_ref();
        let two_t = 2 * self.t;
        ...
        for i_local in 0..r_full {
            indices_full.push(i_local);
        }
        for i_local in r_full..self.n {
            indices_full.push(n_full - self.k + (i_local - r_full));
        }
        ...
        let omega: Poly<GFExt<GFp<2>>> =
            crate::lfsr::error_evaluator_from_syndromes(&synd, &sigma, two_t);
        let dsigma = crate::lfsr::formal_derivative(&sigma);
        // Chien over mapped indices
        let error_pos: Vec<usize> =
            rs_chien_positions_over_indices(&self.field, &sigma, &indices_full);
        let l = error_pos.len();
        if l == 0 {
            // unable to find errors though synd!=0 -> treat as failure
            return Err(CodingError::DecodeFailure {
                text: "no error locations found (nonzero syndromes)".into(),
            });
        }
        if l > self.t {
            return Err(CodingError::DecodeFailure {
                text: "too many errors".into(),
            });
        }
        // Forney: e_i = -Ω(X_i^{-1}) / Σ'(X_i^{-1}); char-2 なので '-' は不要
        let mut corrected = rvec.clone();
        let positions_full: Vec<usize> = error_pos
            .iter()
            .map(|&pos| {
                if pos < r_full {
                    pos
                } else {
                    n_full - self.k + (pos - r_full)
                }
            })
            .collect();
        let values = rs_forney_values(&self.field, &omega, &dsigma, &positions_full)?;
        for (idx, &pos) in error_pos.iter().enumerate() {
            corrected[pos] = corrected[pos].clone() + values[idx].clone();
        }
...
impl ReedSolomon {
    ...
    pub fn new_with_field(k: usize, n: usize, field: &FiniteField2m) -> CodingResult<Self> {
        if n == 0 || k == 0 || k > n {
            return Err(CodingError::InvalidParameters {
                text: format!("invalid (n,k)=({n},{k})"),
            });
        }
        if n > field.n {
            return Err(CodingError::InvalidParameters {
                text: format!("n must be <= 2^m-1 = {}", field.n),
            });
        }
    ...
    pub fn new_auto(k: usize, n: usize) -> CodingResult<Self> {
        ...
        loop {
            ...
            if m > 15 {
                return Err(CodingError::InvalidParameters {
                    text: format!("unsupported n={}, requires m>15 (table limited)", n),
                });
            }
        }
        if m < 2 {
            return Err(CodingError::InvalidParameters {
                text: "m must be >= 2".into(),
            });
        }
