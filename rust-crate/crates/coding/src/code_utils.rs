...
// 線形符号の最小距離（ゼロ語除外）
pub fn linear_hamming_d_min<F: Field + Clone + PartialEq + Zero>(
    codebook: &[Codeword<F>],
) -> usize {
    let n = codebook.first().map(|v| v.as_ref().dim()).unwrap_or(0);
    let mut dmin = n;
...
pub fn formed_g_to_h<F: Field + Clone + PartialEq + Zero + One>(
    g: &GeneratorMatrix<F>,
        ...
        for j in 0..k {
            ...
            if g.0[(i, j)] != exp {
                return Err(CodingError::InvalidParameters {
                    text: "G must be in standard form [I|P]".into(),
                });
            }
        }
    }
...
pub fn to_systematic_g<F: Field + Clone + PartialEq + Zero + One>(
    ...
    for col in 0..n {
        ...
        let pc = match pivot_col {
            Some(x) => x,
            None => {
                return Err(CodingError::RankDeficient);
            }
        };
...
pub fn compute_syndrome_gf2(h: &ParityCheckMatrix<GFp<2>>, r: &Codeword<GFp<2>>) -> Syndrome {
    let n = h.0.cols;
    let m = h.0.rows;
    let v = r.as_ref();
    assert_eq!(v.dim(), n);
...
pub fn syndrome_decode_gf2(
    ...
    Err(CodingError::DecodeFailure {
        text: "No error pattern within t found for given syndrome".into(),
    })
}
