...
pub fn systematic_cyclic_encode_full<F>(
    n: usize,
    k: usize,
    g: &Poly<F>,
    u: &Message<F>,
) -> CodingResult<Codeword<F>>
where
    F: Field + Clone + PartialEq + Zero + One,
{
    if u.dim() != k {
        return Err(CodingError::InvalidParameters {
            text: format!("message length {} must equal k {}", u.dim(), k),
        });
    }
    let r = n - k;
    ...
    if rem.coeffs.len() < r {
        rem.coeffs.resize(r, F::zero());
    }
    let mut c = tmp;
    for i in 0..r {
        c[i] = c[i].clone() + rem.coeffs[i].clone();
    }
    Ok(Codeword::from(Vector::new(c)))
}

// Systematic encode via full-length N then shorten to n_short = r_full + k_short
pub fn systematic_cyclic_encode_shortened_over_full<F>(
    n_full: usize,
    n_short: usize,
    k_short: usize,
    g: &Poly<F>,
    ...
    F: Field + Clone + PartialEq + Zero + One,
{
    if u.dim() != k_short {
        return Err(CodingError::InvalidParameters {
            text: format!("message length {} must equal k {}", u.dim(), k_short),
        });
    }
...
pub fn rs_chien_positions_over_indices(
    ...
    for (i_local, &i_full) in indices_full.iter().enumerate() {
        ...
        if cond_inv || cond_rev {
            error_pos.push(i_local);
        }
    }
    error_pos
}
    ...
    for &i_full in positions_full {
        ...
        if den == GFExt::<GFp<2>>::zero() {
            return Err(CodingError::DecodeFailure {
                text: "singular derivative in Forney".into(),
            });
        }
        out.push(num / den);
    }
...
pub fn bch_generator_poly(field: &FiniteField2m, n: usize, t: usize, b: usize) -> Poly<GFp<2>> {
    assert_eq!(
        n, field.n,
        "For narrow-sense primitive BCH, n should be 2^m-1"
    );
