use coding::Poly;
use coding::{Codeword, CyclicCode, Message, ReedSolomon, GF256};
...
fn cyclic_gf2_lut_roundtrip() {
    ...
    let k = cyc.k();
    let msg = Message::from(Vector::new(
        (0..k).map(|i| GFp::<2>((i as u16) & 1)).collect(),
    ));
    let code = cyc.encode(&msg).expect("encode");
    let mut noisy = code.as_ref().clone();
    noisy[3] = noisy[3] + GFp::<2>(1); // 1-bit error
    let noisy = Codeword::from(noisy);
...
fn bch_gf2_lut_roundtrip_hamming_15_7_like() {
    ...
    r1[4] = r1[4] + GFp::<2>(1);
    let dec1 = bch
        .decode_lut(&Codeword::from(r1))
        .expect("decode_lut 1bit");
    ...
    r2[9] = r2[9] + GFp::<2>(1);
    let dec2 = bch
        .decode_lut(&Codeword::from(r2))
        .expect("decode_lut 2bit");
