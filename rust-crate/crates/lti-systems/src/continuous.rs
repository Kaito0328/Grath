...
use polynomial::prelude::RootMethod;
use std::fmt;
...
impl TransferFunction {
    ...
    pub fn zeros_with(&self, method: RootMethod) -> Vec<Complex<f64>> {
        self.ratio.numerator.find_roots_with(method)
    }
    pub fn poles(&self) -> Vec<Complex<f64>> {
        self.ratio.denominator.find_roots()
    }
    pub fn poles_with(&self, method: RootMethod) -> Vec<Complex<f64>> {
        self.ratio.denominator.find_roots_with(method)
    }
...
impl TransferFunction {
    ...
    pub fn display(&self) -> RfDisplay<'_, f64> {
        self.ratio.display_with("s")
    }
    /// 変数名を指定して可読表示するラッパーを返す
    pub fn display_with(&self, var: &'static str) -> RfDisplay<'_, f64> {
