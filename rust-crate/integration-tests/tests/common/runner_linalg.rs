...
pub fn run_dynamic_test(func_key: &str, inputs: &[String]) -> std::result::Result<String, String> {
    match func_key {
        ...
        "RationalMatrixDtoApi::inverse" => {
            let arg0: RationalMatrixValue = serde_json::from_str(&inputs[0])
        ...
        "RationalMatrixDtoApi::add" => {
            let arg0: RationalMatrixValue = serde_json::from_str(&inputs[0])
                .map_err(|e| format!("Parse DTO arg0 error: {}", e))?;
            let arg1: RationalMatrixValue = serde_json::from_str(&inputs[1])
                .map_err(|e| format!("Parse DTO arg1 error: {}", e))?;
