...
impl WasmPolynomialSolver {
    pub fn inner(&self) -> &PolynomialSolver { &self.0 }
}
...
impl WasmPolynomialApi {
    pub fn inner(&self) -> &PolynomialApi { &self.0 }
}
...
impl WasmPolynomialApi {
    pub fn find_roots_symbolic_expr(coeffs: String) -> std::result::Result<String, JsError> {
        PolynomialApi::find_roots_symbolic_expr(coeffs).map_err(|e| js_error_from_to_app_error(e, None))
    }
}
