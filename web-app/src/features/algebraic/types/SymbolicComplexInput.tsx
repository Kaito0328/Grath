...
export interface SymbolicComplexInputProps {
  label?: ReactNode;
  value: SymbolicComplex | null;
  onChange: (value: SymbolicComplex) => void;
  onRequestSave: (args: { value: string; latex?: string }) => void;
  showActions?: boolean;
  frame?: boolean;
}

export const SymbolicComplexInput = ({
  label,
  value,
  onChange,
  onRequestSave,
  showActions = true,
  frame = true,
}: SymbolicComplexInputProps) => {
  const [busy, setBusy] = useState(false);

  const canUse = !busy && value !== null && value.toString().trim().length > 0;
  ...
  async function onPickVariable(valueText: string) {
    setBusy(true);
    try {
      const parsed = await SymbolicComplex.fromString(valueText);
      onChange(parsed);
    } finally {
      setBusy(false);
    }
  ...
  const content = (
    <Stack gap={"md"}>
      ...
      <Input value={value ? value.toString() : ""} onChange={async (e) => onChange(await SymbolicComplex.fromString(e.target.value).catch(() => value as SymbolicComplex))} placeholder="1 + i" />
    </Stack>
  );
