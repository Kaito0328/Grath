...
Static TypeScript files begin with `// @ts-nocheck` because they intentionally
reference generated-only modules. Inspector removes that directive when it
copies them to the generated SDK, where normal type checking applies. For every
other file, the static scaffold is only a bootstrap source for files that are
missing from the generated SDK. In particular, static sync must not overwrite
generated crate wrappers, generated tests, or `wasm-pkg`.

This keeps hand-written helpers and generated API surfaces separate:

- Generated wrappers, including the algebraic DTO bindings, are recreated from
  `rust-crate/api-specs`.
